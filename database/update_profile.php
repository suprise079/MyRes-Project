...
if (!empty($del)){
	$sql = "UPDATE accomodation set verified = 'N' where Res_ID = '$mainID'";
	$delete = mysqli_query($conn,$sql) or die("Failed to execute: ".mysqli_error($conn));
	$sql = "DELETE FROM users WHERE Res_ID = '$del';";
	$delete = mysqli_query($conn,$sql) or die("Failed to execute: ".mysqli_error($conn));
}
...
if (!empty($price)){
	$sql = mysqli_query($conn, "UPDATE accomodation SET Price_Accreditation = '$price' WHERE Res_ID = '$mainID'");
}
